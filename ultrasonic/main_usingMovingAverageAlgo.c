...
int main() {
    ...
    float sample[10];// collect sample in array
    ...
    while (true) {
        ...
        sample[counterForSamples]=C_distance; //store reading into array
        for (int i=0;i<=10;i++){ // sum of 10 samples in array
            sum+=sample[i];
        }
        
        counterForSamples+=1; 
        avg=sum/counterForSamples; // average the 10 samples
        if (counterForSamples ==10){ // reset counter after the array is full
            counterForSamples=0;
            
        }
        sum=0; // reset the sum after each reading
        printf(" \n%5.2f", avg ); //prints the moving average of center sensor
        // printf(" \n%5.2f", C_distance );
        // if (C_distance>400){
        //     printf("\n C Out of range.");
