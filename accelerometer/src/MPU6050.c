...
void MPU6050_ReadData(int16_t accelerometer[3])
{
    ...
    uint8_t reg = 0x3B; //Register for accelerometer readings
    i2c_write_blocking(I2C_PORT, MPU6050_ADDRESS, &reg, sizeof(reg), false ); //Pass the register to MPU6050
    i2c_read_blocking(I2C_PORT, MPU6050_ADDRESS, buffer, sizeof(buffer), false ); //retrieve the readings from MPU6050

    //populate the accelerometer array with x, y, z values
